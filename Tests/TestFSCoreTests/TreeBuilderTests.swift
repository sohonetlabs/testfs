...
final class TreeBuilderTests: XCTestCase {
    ...
    func testCaseFoldCollisionFailsBuild() {
        let root: TreeNode = .directory(
            name: "r",
            contents: [
                .file(name: "Foo", size: 1),
                .file(name: "foo", size: 2)
            ])
        XCTAssertThrowsError(try TreeBuilder.build(root: root, options: defaultOptions())) { error in
            guard let err = error as? TreeBuilder.BuildError,
                case .caseFoldCollision(_, let folded, let existing, let incoming) = err
            else {
                return XCTFail("expected caseFoldCollision, got \(error)")
            }
            XCTAssertEqual(folded, "foo")
            XCTAssertEqual(Set([existing, incoming]), Set(["Foo", "foo"]))
        }
    }

    func testCaseFoldCollisionErrorIncludesFullPath() {
        ...
        XCTAssertThrowsError(try TreeBuilder.build(root: root, options: defaultOptions())) { error in
            guard let err = error as? TreeBuilder.BuildError,
                case .caseFoldCollision(let directory, _, _, _) = err
            else {
                return XCTFail("expected caseFoldCollision, got \(error)")
            }
            XCTAssertEqual(
    ...
    func testFileSizesPreserved() throws {
        ...
        XCTAssertEqual(index.lookup(name: "huge", in: index.rootID)?.size, UInt64.max)
    }
    ...
    // MARK: - case sensitivity (matches Python upstream, issue #14)
